...
export class ManageSlotsComponent implements OnInit {
  ...
  private initializeCalendar(): void {
    this.calendarOptions = {
      ...
      eventOverlap: false,
      eventAllow: this.allowEventMove.bind(this) // modularized helper
    };
  }
  ...
  loadDoctors(): void {
    ...
    this.adminService.getAllDoctorsList(Filter).subscribe({
      next: (res: any) => {
        this.doctors = (res.data || res.Data || []).map((d: any) => ({
          ...d,
  ...
  // ====================== EVENT ALLOW LOGIC ======================
  /**
   * called by FullCalendar to allow/disallow a move or resize before it happens.
   * dropInfo: { start: Date, end: Date, ... }
   * draggedEvent: the event object being moved/resized; we read its extendedProps.slot to identify the original slot.
   */
  private allowEventMove(dropInfo: any, draggedEvent: any): boolean {
    ...
    // Prefer the original slot object from the event's extendedProps if available.
    const originalSlot: DoctorSchedule | undefined = draggedEvent?.extendedProps?.slot;

    const slotToTest: DoctorSchedule = {
      DoctorScheduleID: originalSlot?.DoctorScheduleID, // may be undefined for new unsaved slots
    ...
    // IMPORTANT: pass originalSlot to isOverlapping so we don't treat the slot itself as overlapping.
  ...
  onEventChange(changeInfo: any): void {
    // changeInfo can be from drop or resize
    ...
    // IMPORTANT: pass original slot to be ignored during overlap check
    ...
    // find by DoctorScheduleID if available, else fallback to matching by time/day
  ...
  // ====================== VALIDATION ======================
  /**
   * isOverlapping(newSlot, ignoreSlot?)
   * newSlot: candidate slot to test
   * ignoreSlot: if provided, that slot will be ignored during overlap checks (useful when updating the same slot).
   */
  private isOverlapping(newSlot: DoctorSchedule, ignoreSlot?: DoctorSchedule | undefined): boolean {
    return this.doctorSchedules.some(existing => {
      // skip exact same object reference (defensive)
      if (existing === newSlot) return false;

      // if we have an ignoreSlot, skip matching by ID or by time/day match
      if (ignoreSlot) {
        // If both have IDs and they match, skip
        ...
        // fallback: same day & same start/end times (previous state) -> skip
      ...
      // overlap check: true if intervals intersect
...
//   loadDoctors(): void {
//     // this.adminService.getDoctorsDropdown().subscribe(
//     //   (res: any) => {
//     //     debugger;
//     //     this.doctors = res.data || []
//     //   },
//     //   (err) => {
//     //     console.error(err); this.doctors = [];
//     //     this.toaster.error('Failed to load doctors');
//     //   }
//     // );
//     const Filter: any = {
//       PageSize: 100,
...
//       (res: any) => this.slotDurations = res.data || [],
//       (err) => {
//         console.error(err); this.slotDurations = [];
//         this.toaster.error('Failed to load Slot Durations');
//       }
//     );
//   }
...
//     // Preventign  hte overlapping slots.
//     if (this.isOverlapping(updatedSlot)) {
//       this.toaster.error('This slot overlaps with another existing slot!');
//       changeInfo.revert();
//       return;
//     }

//     const idx = this.doctorSchedules.findIndex(s => s === slot);
//     if (idx > -1) this.doctorSchedules[idx] = updatedSlot;
//     changeInfo.event.setProp('title', this.getSlotTitle(updatedSlot));
//     changeInfo.event.setExtendedProp('slot', updatedSlot);
//   }
...
//     this.adminService.saveDoctorSchedule(payload).subscribe(
//       (res: any) => res.success ? this.toaster.success(res.message) : this.toaster.error(res.message),
//       (err) => {
...
//   private isOverlapping(newSlot: DoctorSchedule): boolean {
//     return this.doctorSchedules.some(existing => {
//       if (existing === newSlot) return false; // skip self
//       if (existing.DayOfWeek !== newSlot.DayOfWeek) return false;
